...
func _ready():
	...
	if player == 2:
		zhin_body.collision_layer = 2
		zhin_body.collision_mask = 2
...
func _physics_process(delta):
	...
	#Check if player is trying to crouching high and if they can, then executes the move
		...
		if !is_hit:
			if attack_range_body != null:
				attack_range_body._hit(2, 0.7, Vector2(10*is_flipped, -50))
	...
	#Check if player is trying to low blow and if they can, then executes the move
		...
		if !is_hit:
			if attack_range_body != null:
				attack_range_body._hit(15, 1.4, Vector2(50*is_flipped, -300))
	...
	if (direction and !is_hit and !is_crouching) and (is_jump_high or is_jump_low or !is_attacking): 
